...
public class KVSClient implements KVS {
  ...
  public void clean() throws IOException {
    ...
    for (WorkerEntry w : workers) {
      try {
        byte[] response = HTTP.doRequest("PUT", "http://"+w.address+"/clean/", null).body();
        String result = new String(response);
      } catch (Exception e) {}
    }
  }
  ...
  public void tableGC(String oldTableName, String newTableName) throws IOException {
    if (!haveWorkers)
      downloadWorkers();

    for (WorkerEntry w : workers)
      HTTP.doRequest("PUT", "http://"+w.address+"/tableGC/"+java.net.URLEncoder.encode(oldTableName, "UTF-8"), newTableName.getBytes());
  }
